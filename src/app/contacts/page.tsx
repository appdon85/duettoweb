...
export default function ContactsPage() {
  ...
  // Empresas disponiveis para vincular e para o filtro (lista leve, carregada uma vez).
  ...
  const load = useCallback(
    async (targetPage: number, targetSearch: string, targetCompanyId: string) => {
  ...
  useEffect(() => {
    ...
    load(page, search, companyFilter);
  }, [user, page, search, companyFilter, load]);
  ...
  useEffect(() => {
    ...
    authFetch<Paginated<Company>>("/companies?pageSize=100")
      ...
      .catch(() => {
        // Lista de empresas e' usada pelo filtro e pelo formulario de vinculo -
        // uma falha aqui nao deve travar a pagina de contatos inteira.
      });
  }, [user, authFetch]);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      closeDrawer();
      await load(page, search, companyFilter);
    } catch (err) {
    ...
    <AppShell>
      <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between gap-4">
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">Contatos</h1>
          {canManage && (
            <button
              onClick={openCreate}
