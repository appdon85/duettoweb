...
import type { Company, Contact, Deal, DealStage, Paginated, TenantUserRow } from "@/lib/types";
...
export default function DealsPage() {
  ...
  const load = useCallback(
    async (targetSearch: string, targetCompanyId: string) => {
      try {
  ...
  useEffect(() => {
    ...
    load(search, companyFilter);
  }, [user, search, companyFilter, load]);
  ...
  useEffect(() => {
    // Lista de usuarios do tenant para o campo "Responsavel" (owner) - so'
    // carrega se o papel tiver users:read (ver PERMISSIONS.USERS_READ); sem
    // essa permissao o campo simplesmente nao aparece no formulario.
  ...
  function ownerName(id: string | null): string | null {
    if (!id) return null;
    return userOptions.find((u) => u.id === id)?.name ?? null;
  }
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      closeDrawer();
      await load(search, companyFilter);
    } catch (err) {
    ...
    <AppShell>
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between gap-4">
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">Negócios</h1>
          {canManage && (
            <button
              onClick={openCreate}
                        ...
                        return (
                          ...
                          >
                            ...
                            {ownerName(deal.ownerUserId) && (
                              <p className="text-xs text-zinc-500">
                                Responsável: {ownerName(deal.ownerUserId)}
                              </p>
                            )}
